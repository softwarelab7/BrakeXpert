...
const ProductCard = React.memo(({ product }: ProductCardProps) => {
    ...
    const highlightMap = useMemo(() => {
        ...
        for (const match of result.matches) {
            if (['referencia', 'ref', 'oem', 'fmsi', 'wva'].includes(match.key)) {
                map.set(match.value, [...(match.indices as [number, number][])]);
            }
        }
        return map;
        ...
        <div className={`product-card ${getHoverClass()}`} onClick={() => openProductDetailModal(product.id)}>
            ...
            <div className="ref-section">
                {(product.referencia ? [product.referencia] : (product.ref || []))
                    ...
                    .filter(Boolean)
                    .map((reference, idx) => {
                        const indices = highlightMap.get(reference);
                        const segments = indices
                            ? buildHighlightSegments(reference, indices)
                            : [{ text: reference, highlight: false }];
                        ...
                        return (
                            ...
                            >
                                {segments.map((seg, j) =>
                                    seg.highlight
                                        ? <mark key={j} className="ref-highlight">{seg.text}</mark>
